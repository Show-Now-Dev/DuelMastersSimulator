...
var CardEditor = (function () {
  ...
  function _addHyperspatialInputs(container, preview, saveBtn, setParsed) {
    ...
    addFaceBtn.addEventListener('click', function () {
      ...
      var ta3 = _makeTA('面3（3D龍解 / ○○・スーパー・クリーチャー など）');
      taObjs.push(ta3);
    ...
    container.appendChild(_buildSampleWidget({
      ...
      faces: [
        ...
        {
          nameHTML: '《<ruby>勝首領龍<rt>ブイブイブイドラゴン</rt></ruby> カツドン》',
          rows: [
            '勝首領龍 カツドン　SPR　火文明　(4)',
            'サイキック・クリーチャー：アウトレイジMAX　4000',
        ...
        // ── 面2（覚醒後 / フォートレス など） ───────────────────────────────
        {
          nameHTML: '《<ruby>勝首領将軍<rt>ブイブイブイショーグン</rt></ruby> カツキング》',
          rows: [
            '勝首領将軍 カツキング　SPR　火文明　(6)',
            'サイキック・クリーチャー：アウトレイジMAX　7000',
            'W・ブレイカー',
            ...
            '解除（このクリーチャーが離れる時、かわりにコストが小さいほうに裏返す）',
                ...
                { divider: '── 面3（それぞれのテキストボックスへ別々に貼り付け）──' },
        // ── 面3（3D龍解 / 3段階覚醒 など） ───────────────────────────────
        {
          nameHTML: '《<ruby>最終勝首領<rt>ブイブイブイファイナル</rt></ruby> カツマスター》',
          rows: [
  ...
  function _addSuperGRInputs(container, preview, saveBtn, setParsed) {
    ...
    container.appendChild(_buildSampleWidget({
      faces: [
        {
          nameHTML: '《“<ruby>魔神轟怒<rt>マジゴッド</rt></ruby>”ブランド》',
          rows: [
            '“魔神轟怒”ブランド　MAS　火文明　(5)',
